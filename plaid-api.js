...
async function syncTransactions(userId, accessToken, cursor) {
  let newCursor = cursor; //cursor to only retrieve new/modified/deleted tx
  let added = [];
  let modified = [];
  let removed = [];
  let hasMore = true;
  //loop through response and add transactions to arrays
  while (hasMore) {
...
export async function POST(request) {
  ...
  //Step 1 of process --> user retrieves link-token
  ...
  // exchange the user's public token to get access-token (to access bank acct in future)
